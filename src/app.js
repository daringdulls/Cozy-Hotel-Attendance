...
const staff = [
  { id: 1, name: 'Aishath Naya', role: 'Front Office', initials: 'AN', off: 'Friday', rate: 52, color: '#d87e5f' },
  { id: 2, name: 'Ibrahim Zayan', role: 'Operations', initials: 'IZ', off: 'Monday', rate: 58, color: '#73907f' },
  { id: 3, name: 'Mariyam Leena', role: 'Housekeeping', initials: 'ML', off: 'Tuesday', rate: 48, color: '#c49a50' },
  { id: 4, name: 'Ahmed Rilwan', role: 'Food & Beverage', initials: 'AR', off: 'Thursday', rate: 50, color: '#6e89a8' },
  { id: 5, name: 'Fathimath Aya', role: 'Finance', initials: 'FA', off: 'Saturday', rate: 62, color: '#9c7b9f' },
];
    ...
    <div class="shell">
      <aside>
        ...
        <div class="aside-bottom">
          ...
          <div class="profile"><span class="avatar small">HM</span><div><b>Hana Moosa</b><small>Administrator</small></div><button>•••</button></div>
        </div>
      </aside>
      <main>
...
function dashboardView(){
  return `<section class="content">
    ...
    <div class="title-row"><div><h1>Good morning, Hana.</h1><p>Here’s how your team is doing today.</p></div><button class="primary" id="addStaff">${icon('plus')} Add staff member</button></div>
    <div class="metrics">
      ${metric('Today’s attendance','4 / 5','80% present','up')}
    ...
    <div class="grid-two">
      <article class="card attendance-card">
        ...
        ${staff.map(s=>{const r=records[s.id], h=hours(r); return `<tr><td><div class="person"><span class="avatar" style="--avatar:${s.color}">${s.initials}</span><div><b>${s.name}</b><small>${s.role}</small></div></div></td><td>${r[0]}</td><td>${r[1]||'—'}</td><td>${h?h.toFixed(1)+'h':'—'}</td><td>${s.id===5?'<span class="pill working">Working</span>':s.id===3?'<span class="pill late">Late</span>':'<span class="pill present">Present</span>'}</td></tr>`}).join('')}
        </tbody></table></div>
      </article>
      <aside class="side-stack">
        <article class="card schedule"><div class="card-head"><div><h2>Weekly schedule</h2><p>03 – 09 August</p></div></div>
          ...
          <div class="off-list"><div><span class="avatar small" style="--avatar:#d87e5f">AN</span><p><b>Aishath Naya</b><small>Day off · Friday</small></p><span class="tag">OFF</span></div><div><span class="avatar small" style="--avatar:#6e89a8">AR</span><p><b>Ahmed Rilwan</b><small>Day off · Thursday</small></p><span class="tag">OFF</span></div></div>
        </article>
